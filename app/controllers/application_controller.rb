class ApplicationController < ActionController::Base
  ...
  # 调用微信授权获取openid
  def invoke_wx_auth
    if params[:state].present? && params[:state] == oauth.state \
  ...
  def get_wechat_sns
    ...
    if session[:openid].blank? && params[:state].present? && params[:state] == oauth.state
      client = WechatAuth::Client.new(code: params[:code])
      sns_info = client.json_hash
